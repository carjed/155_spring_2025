---
title: "19/20. Sampling distributions, the CLT, and Bootstrapping"
...
-   You can download a template file to work with [here](../activity_templates/19_20_sampling_dist_clt_bootstrap.qmd).
...
-   We can *approximate* the sampling distribution and standard error using bootstrapping. BUT:
...
Neither approach is perfect, but they complement one another. **Bootstrapping** in particular, while it cannot and should not *replace* the CLT, gives us some nice intuition behind the idea of resampling, which is fundamental for hypothesis testing (which we'll get to shortly!).
...
The code below obtains *500* separate samples of 10 fish, and stores the model estimates from each. (You will first need to run `install.packages("mosaic")` in the Console):
...
-   \_\_\_% of samples will produce $\hat{\beta}_1$ estimates within **1 st. err.** of $\beta_1$

-   \_\_\_% of samples will produce $\hat{\beta}_1$ estimates within **2 st. err.** of $\beta_1$

-   \_\_\_% of samples will produce $\hat{\beta}_1$ estimates within **3 st. err.** of $\beta_1$
...
a.  For all sample sizes, the shape of the sampling distribution is roughly \_\_\_ and the sampling distribution is roughly centered around \_\_\_, the sample estimate from our original data.
...
[Solutions](../solutions/19_20_sampling_dist_clt_bootstrap.qmd)
